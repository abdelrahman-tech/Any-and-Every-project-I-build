...
def img_to_pix(filename):
    ...
    """
    data = Image.open(filename).getdata()
    return list(data)
...
def filter(pixels_list, color):
    ...
    """
    colorMatrix = make_matrix(color)
    resultPixels = ()
    for pixel in pixels_list:
        i = tuple(matrix_multiply(colorMatrix, pixel)) #temporary variable
        result = (int(i[0]), int(i[1]), int(i[2])) # this is valid because the function only takes RBG list
        resultPixels += (result, )
    return list(resultPixels)
...
def reveal_bw_image(filename):
    ...
    filename_pixels = img_to_pix(filename)
    filename_width, filename_height = main_img.size
    filename_mode = main_img.mode
    main_img.close()
    
    LSB_pixel = []
    for pixel in filename_pixels:
        LSB_pixel.append(extract_end_bits(1, pixel))
    result_img = pix_to_img(LSB_pixel, (filename_width, filename_height), filename_mode)
    result_img.putdata(LSB_pixel, scale=255)
    return result_img
...
def reveal_color_image(filename):
    ...
    """
    main_img = Image.open(filename)
    filename_pixels = img_to_pix(filename) #list of tuples
    filename_width, filename_height = main_img.size
    filename_mode = main_img.mode
    main_img.close()
    
    LSB_pixel = [] #list of tuples
    for pixel in filename_pixels:
        #pixel is tuple of 3 digits
        px1 = extract_end_bits(3, pixel[0])
        px2 = extract_end_bits(3, pixel[1])
        px3 = extract_end_bits(3, pixel[2])
        tmp = (px1, px2, px3)
        LSB_pixel.append(tmp)
    
    result_img = pix_to_img(LSB_pixel, (filename_width, filename_height), filename_mode)
    result_img_px = result_img.getdata()
    new_px = []
    for pixel in LSB_pixel:
        new_px.append((pixel[0] * 24, pixel[1] * 24, pixel[2] * 24))
    final_img =  pix_to_img(new_px, (filename_width, filename_height), filename_mode)
    return final_img
...
def main():
    ...
    im2.show()
    im = reveal_color_image('hidden2.bmp')
    im.show()
